...
void GirarRobo(int degree, int sentido){
 ...
 //than 90 degrees (from latest reset position)
	 repeatUntil(getGyroHeading(S4) > 90){
...
** Param: cor
*/
void estado_plaza (TLegoColors cor){
	while (cor == colorWhite){//Anda para frente com uma distância de no mínimo 50cm e máximo de 100cm de distância da parede.
	...
	if(cor == colorBlack){	//Anda 30cm, abre a cancela anda 50cm para trás, fecha a cancela e gira 180°
		anda_x_cm (30);
		...
		setMotorSpeed(dir, 0);
		setMotorSpeed(esq,0);
	}else{//Caso o sensor não ache a cor preta, quer dizer que o sensor não está funcionando e então andamos reto e voltamos para o começo da função
		estado_plaza ();
	}

	while (cor == colorWhite){//Volta para a rampa, tentando manter a distância minima de 50cm e máxima de 100cm da parede
		if (getUSDistance(ult) > 100){
			setMotorSpeed(dir, 18);
			setMotorSpeed(esq, 22);
		}else if (getUSDistance(ult) < 50) {
